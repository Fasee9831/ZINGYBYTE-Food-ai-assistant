...
def _render_metrics(metrics: dict, content: str) -> None:
    """Render metric badges below an assistant message."""
...
def render_chat_bubble(role: str, content: str, metrics: dict = None) -> None:
    """Render a single chat message using Streamlit's native component."""
    with st.chat_message(role):
        st.markdown(content)
        if metrics and role == "assistant":
            _render_metrics(metrics, content)
