...
def inject_premium_styles() -> None:
    ...
    st.markdown(
        ...
        /* Row container — flex layout with avatar alignment */
        .zb-chat-row {{
            display: flex;
            align-items: flex-start;
            gap: 10px;
        ...
        /* User messages — avatar on the right, bubble before it */
        .zb-chat-row.zb-row-user {{
            flex-direction: row-reverse;
        }}

        /* Assistant messages — avatar on the left, bubble after it */
        .zb-chat-row.zb-row-assistant {{
            flex-direction: row;
        }}
        ...
        .zb-chat-row:hover .zb-bubble-avatar {{
            transform: scale(1.08);
            box-shadow: 0 4px 16px rgba(255,133,51,0.2);
        }}

        /* User avatar gets a warm brand glow */
        .zb-row-user .zb-bubble-avatar {{
            background: linear-gradient(135deg, rgba(255,75,75,0.15), rgba(255,133,51,0.12));
            border-color: rgba(255,133,51,0.2);
        }}

        /* Bubble body — the actual message card */
        .zb-bubble {{
            max-width: 78%;
            padding: 13px 18px;
            border-radius: 18px;
        ...
        /* User bubble — brand gradient, right side */
        .zb-bubble-user {{
            background: linear-gradient(135deg, #FF4B4B 0%, #FF8533 100%);
            color: #fff;
            border-bottom-right-radius: 4px;
            box-shadow:
                0 4px 20px rgba(255,75,75,0.22),
                0 1px 0 rgba(255,255,255,0.08) inset;
        }}
        .zb-bubble-user:hover {{
            transform: translateY(-1px);
            box-shadow:
                0 8px 30px rgba(255,75,75,0.3),
                0 1px 0 rgba(255,255,255,0.1) inset;
        }}
        .zb-bubble-user .zb-bubble-content p {{
            color: #fff !important;
        }}
        .zb-bubble-user .zb-bubble-content,
        .zb-bubble-user .zb-bubble-content * {{
            color: #fff;
        }}

        /* Assistant bubble — dark glassmorphic, left side */
        .zb-bubble-assistant {{
            background: linear-gradient(145deg,
        ...
        .zb-bubble-content p:last-child {{
            margin-bottom: 0;
        }}
        .zb-bubble-content ul,
        .zb-bubble-content ol {{
            margin: 6px 0;
            padding-left: 1.4rem;
        }}
        .zb-bubble-content li {{
            margin-bottom: 4px;
        }}
        .zb-bubble-content strong {{
            font-weight: 600;
        }}

        /* Streaming cursor inside assistant bubble */
        .zb-cursor {{
        ...
        /* Metric badges rendered inside / below bubbles */
        .zb-bubble-metrics {{
            display: flex;
            flex-wrap: wrap;
            gap: 5px;
        ...
        .zb-bubble-user .zb-bubble-metrics {{
            border-top-color: rgba(255,255,255,0.15);
        }}

        /* Hide the old Streamlit chat message chrome */
        .stChatMessage {{
        ...
        @media (max-width: 768px) {{
            ...
            /* Ensure sidebar toggle button is visible on dark bg */
            ...
            /* Force 2-column Streamlit grids to single column on mobile */
            [data-testid="column"] {{
            ...
            /* Floating food — kept at full visibility on mobile */
